...
/**
 * Servicio para tratar el formulario: Isla
 * 
 * @author agarcia@at4.net
 * 
 */
@Service("isla")
public class IslaService extends BaseService<IslaBO, IslaListadoVO, IslaFormVO> {

	/**
	 * The logger object.
	 */
	private static Logger log = LogService.getLogger(IslaService.class);
	 ...
	 */
	@Override
	protected boolean hasPerms(BaseFrontPrincipal usuari, String action, IslaFormVO instance) {
	...
	public GridMetaData getListMetadata(FormItem formItem, BaseFrontPrincipal usuari, Locale locale) {
		...
		if ((formItem.getText() == null) || formItem.getText().equals("")) {
			formItem.setText(this.getMessage("forms.isla.title", locale));
		}

		GridMetaData metaData = new GridMetaData(formItem);

		//Nombre de la clase js de formulario
		metaData.setFormClassName("IslaForm");
		...
		/** Creamos la columna */
		metaData.addColumn(new GridColumnMetaData("id", this.getMessage("forms.isla.columns.id.header", locale)));
		...
		/** Creamos la columna */
		metaData.addColumn(new GridColumnMetaData("nombre", this.getMessage("forms.isla.columns.nombre.header", locale)));
	...
	public DatosExportacion getDatosExportacion(Locale locale) throws GenericBusinessException {
		...
		datosExportacion.add("id", this.getMessage("forms.isla.columns.id.header", locale), TipoCampoExportacion.NUMBER);
		datosExportacion.add("nombre", this.getMessage("forms.isla.columns.nombre.header", locale), TipoCampoExportacion.STRING);
	...
	public List<String> listadoToExportRow(ListadoVO rawInstance, Locale locale) {
		IslaListadoVO instance = (IslaListadoVO) rawInstance;
	...
	/**
	 * Rellena una instancia de IslaFormVO con el contenido de un formulario
	 * recibido en formato Map<String, ?>
	 */
	protected void setFormData(IslaFormVO record, Map<String, ?> formData) throws GenericBusinessException {
		String formDataStr = null;
		if (FormData2Java.needsUpdate(formData, "id")) {
